...
def pretrain_validation(index):
    ...
    if check_write_log():
        ...
        logger.info("validation_loss over epochs epoch=%d, val_loss=%f"%(index, np.float(eval_loss)))
    return eval_loss
        ...
        try:
            ...
            if (step + 1) % gradient_accumulation_steps == 0:
                ...
                if check_write_log() and (global_step%args.log_steps == 0):
                    logger.info("training_loss %f"%np.float(global_step_loss))
                    logger.info("lr_this_step %f"%np.float(lr_this_step))
                    logger.info("loss over steps=%f, loss=%f"%(global_step, np.float(global_step_loss)))
                    logger.info("lr over steps=%f, lr=%f"%(global_step, np.float(lr_this_step)))
                global_step_loss = 0
...
def check_write_log():
    #print('dist get rank: ', dist.get_rank())
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model", default="bert", type=str, 
                        help="The model used for pretaining. Currently only support bert or electra.")
    ...
    logger.info("host: {} device: {} n_gpu: {}, use_multigpu_with_single_device_per_process: {}, 16-bits training: {}".format(
        hostname, device, n_gpu, use_multigpu_with_single_device_per_process, fp16))
    ...
    if args.output_dir is None:
        parent_dir = os.path.join(args.output_dir, 'outputs', str(experiment_name))
        output_dir = os.path.join(parent_dir, str(run_id))
    ...
    if check_write_log():
        ...
        summary_writer = get_sample_writer(
            name=job_name+str(local_rank), base='./logs')
    ...
    model = BertMultiTask(model_name=args.model, job_config = job_config, use_pretrain = use_pretrain, tokenizer = tokenizer, 
    ...
    for index in range(start_epoch, args.epochs):
        ...
        if check_write_log():
            if best_loss is None or eval_loss is None or eval_loss < best_loss*0.99:
                best_loss = eval_loss
                epoch_ckp_path = os.path.join(saved_model_path, "{}_{}_bert_encoder_epoch_{:04d}.pt".format(job_id, local_rank, index + 1))
                checkpoint_model(os.path.join(saved_model_path, "{}_{}_training_state_checkpoint_{:04d}.tar".format(job_id, local_rank, index + 1)), model, optimizer, index, global_step)
                ...
                if args.best_cp_dir:
                    best_ckp_path = os.path.join(args.best_cp_dir, "{}_{}_bert_encoder_epoch_{:04d}.pt".format(job_id, local_rank, index + 1))
            ...
            if args.latest_cp_dir:
                ...
                os.makedirs(args.latest_cp_dir,exist_ok=True)
                checkpoint_model(os.path.join(args.latest_cp_dir, "{}_{}_training_state_checkpoint_{0:04d}.tar".format(job_id, local_rank, index + 1)), model, optimizer, index, global_step)
                latest_ckp_path = os.path.join(args.latest_cp_dir, "{}_{}_bert_encoder_epoch_{:04d}.pt".format(job_id, local_rank, index + 1))
