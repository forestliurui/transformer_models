...
class BertForSequenceRegression(BertPreTrainedModel):
    ...
    Inputs:
        ...
        `labels`: labels for the regression output: torch.LongTensor of shape [batch_size]
            with indices selected in [0, ..., num_labels].

    Outputs:
        if `labels` is not `None`:
            Outputs the MSE regression loss of the output with the labels.
        if `labels` is `None`:
            Outputs the regression logits of shape [batch_size, num_labels].
...
class ElectraForSequenceRegression(ElectraPreTrainedModel):
    def __init__(self, config):
